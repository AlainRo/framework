...
let dir = []
try {
  dir = readdirSync('./cache')
  file = readdirSync('./cache').filter(fileName => fileName.split('.')[0]==='history')[0]
  if (file===undefined){file = readdirSync('./cache').filter(fileName => fileName.split('.')[0]==='history')[0]}
  if (file===undefined) {file = history.json}
  const historyF = readFileSync('./cache/'+file)
